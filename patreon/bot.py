...
from buttons import auth_button, main_menu_keyboard, login_chat, donate_bot, accept_pay, sub_menu
...
async def start(message: types.Message):
    await message.reply(f"Здравствуй!\n\nЭто телеграм бот либертарианского СМИ <a href='https://t.me/svtvnews'>SVTV NEWS</a> для сбора пожертвований.\n\n"\
    f"Здесь вы можете оформить единовременный донат или подписку на несколько месяцев.",parse_mode='HTML', reply_markup=main_menu_keyboard, disable_web_page_preview=True)

@dp.message_handler(text=['💵 Отправить донат'])
async def send_donate(message: types.Message):
    await message.reply(f"Из-за санкционных ограничений, пользователям из России могут быть недоступны зарубежные сервисы по приёму платежей — и наоборот." \
...
async def svtv_comm_state(message: types.Message, state: FSMContext):
    try:
        ...
        await bot.send_message(message.chat.id, f"<strong>{datetime.strftime(datetime.now(msk), '%d.%m.%Y %H:%M')}</strong>\n\n" \
...
async def on_startup(dp):
    try:
        ...
        scheduler.add_job(activday2, "cron", hour="13", minute="0", day="1") # для проверки и исключения тех, кто оформлял подписку в боте
        scheduler.start()
    except Exception as e:
